...
async function showAllUsers() {
  console.log('\n========================================================================================');
  console.log('              DOCISCAN - ALL REGISTERED OFFICERS & USERS (SUPABASE DB)');
  console.log('========================================================================================\n');

  try {
    const result = await pool.query(
      'SELECT id, name, email, mobile, role, status, to_char(created_at, \'YYYY-MM-DD HH24:MI:SS\') as registered_on FROM users ORDER BY created_at DESC'
    );
    ...
    if (result.rows.length === 0) {
      ...
      console.log(`\nTotal Registered Users in Database: ${result.rows.length}`);
    }
  } catch (err) {
